...
public sealed class RedactionValidationAndAdapterTests
{
    ...
    public void Gateway_options_validator_reports_all_invalid_startup_configuration()
    {
        ...
        // Minimal-config validation: explicitly opt the broker out, since the broker
        // default-on path requires several secrets that this test deliberately does not
        // supply.  Equivalent to the pre-broker assertion.
        Assert.True(new GatewayOptionsValidator().Validate(null, new GatewayOptions
        {
